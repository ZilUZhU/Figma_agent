...
//   on<RequestFigmaFunctionHandler>('REQUEST_FIGMA_FUNCTION', async (functionCallData: FunctionCallData) => {
//     console.log(`[main.ts] Received request to execute function: ${functionCallData.name}`);
...
//       console.log(`[main.ts] Function ${functionCallData.name} executed. Result length: ${resultString.length}`);
...
//     } catch (error) {
//       console.error(`[main.ts] Error executing function ${functionCallData.name}:`, error);
//       // Send an error result back to the UI
//       emit<FigmaFunctionResultHandler>('FIGMA_FUNCTION_RESULT', {
//         call_id: functionCallData.call_id,
//         output: JSON.stringify({ // Send error details as stringified JSON
//             error: `Failed to execute function ${functionCallData.name}: ${error instanceof Error ? error.message : String(error)}`
//         }),
//       });
//     } finally {
